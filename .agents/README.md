...
section (ARD, tables) contributes its skills here so there is one place to look
and one convention to follow.
...
.agents/
└─ skills/
   ├─ ard-creation/              # one folder per skill
...
---
name: ard-creation
description: Build Analysis Results Datasets with cards and cardx. Use when
  summarizing a clinical dataset into an ARD before rendering a table.
  Triggers on "ARD", "cards", "ard_stack", "ard_summary", "ard_tabulate".
---
...
Note: `slides/ard/cards.qmd` and `slides/gtsummary/08-coding-agents.qmd` read
`ard-creation/SKILL.md` and `gtsummary-tables/SKILL.md` at render time —
renaming or deleting either file breaks the site build.
